...
var parseEs6Import = require('./parse-es6-import').parse;
...
ReplaceRequire.prototype = _.create(Injector.prototype, {
	...
	injectToFile: function(filePath, code, ast) {
		...
		try {
			...
			if (factoryMaps.length > 0) {
				...
				return this.replace(code, factoryMaps, fileParam, ast);
			}
			return code;
		} catch (e) {
		...
		estraverse.traverse(ast, {
			enter: function(node, parent) {
				if (node.type === 'CallExpression') {
					...
					if (calleeType === 'Identifier' || calleeType === 'Import') {
	...
	onImportAsync: function(node, factoryMaps, fileParam, patches) {
		...
		this.addPatch(patches, node.range[0], node.range[1], old, 'ima', factoryMaps, fileParam);
	},
	...
	onRequire: function(node, factoryMaps, fileParam, patches) {
		...
		_.get(node, 'callee.name') === 'require') {
			var old = _.get(node, 'arguments[0].value');
			this.addPatch(patches, node.range[0], node.range[1], old, 'rq', factoryMaps, fileParam);
		}
	},
	...
	onRequireEnsure: function(node, factoryMaps, fileParam, patches) {
		...
		if (args[0].type === 'ArrayExpression') {
			args[0].elements.forEach(nameNode => {
				...
				self.addPatch(patches, nameNode.range[0], nameNode.range[1], old, 'rs', factoryMaps, fileParam);
			});
		} else if (args[0].type === 'Literal') {
			var old = _.get(node, 'arguments[0].value');
			self.addPatch(patches, args[0].range[0], args[0].range[1], old, 'rs', factoryMaps, fileParam);
		}
	},
...
function parseCode(code) {
	...
	var ast;
	try {
		ast = acornjsx.parse(code, {ranges: true, allowHashBang: true, sourceType: 'module', plugins: {jsx: true, dynamicImport: true}});
	} catch (err) {
		ast = acornjsx.parse(code, {ranges: true, allowHashBang: true, plugins: {jsx: true, dynamicImport: true}});
	}
	return ast;
}
