...
var {TypescriptParser} = require('../dist/parse-ts-import');
...
ReplaceRequire.prototype = _.create(Injector.prototype, {
		...
		try {
			...
			if (factoryMaps.length > 0) {
				if (filePath.endsWith('.ts')) {
					...
					return this.tsParser.replaceTs(this, code, factoryMaps, filePath, ast);
				}
	...
	replace: function(code, factoryMaps, fileParam, ast) {
		...
		estraverse.traverse(ast, {
			enter: function(node, parent) {
				...
				} else if ((node.type === 'ExportNamedDeclaration' && node.source) || (node.type === 'ExportAllDeclaration' && node.source)) {
	...
	onExport: function(node, factoryMaps, fileParam, patches) {
	...
	addPatch: function(patches, start, end, moduleName, replaceType, fmaps, fileParam) {
		var self = this;
		var setting;
