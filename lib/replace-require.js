...
ReplaceRequire.prototype = _.create(Injector.prototype, {
	...
	injectToFile: function(filePath, code, ast) {
		...
		try {
			...
			if (dir) {
				...
				return this.replace(code, factoryMap, fileParam, ast);
			}
			return code;
		} catch (e) {
	...
	replace: function(code, factoryMap, fileParam, ast) {
		var self = this;
		if (!ast) {
			ast = parseCode(code);
		}
			...
			enter: function(node, parent) {
				if (node.type === 'CallExpression') {
					...
					if (calleeType === 'Identifier' && _.get(node, 'callee.name') === 'require') {
						self.onRequire(node, factoryMap, fileParam, patches);
					} else if (calleeType === 'MemberExpression' &&
						node.callee.object.name === 'require' &&
						node.callee.object.type === 'Identifier' &&
						node.callee.property.name === 'ensure' &&
						node.callee.property.type === 'Identifier') {
						self.onRequireEnsure(node, factoryMap, fileParam, patches);
					}
				}
			},
	...
	onRequire: function(node, factoryMap, fileParam, patches) {
		var calleeType = _.get(node, 'callee.type');
		if (calleeType === 'Identifier' &&
		_.get(node, 'callee.name') === 'require') {
			var old = _.get(node, 'arguments[0].value');
			this.addPatch(patches, node.range[0], node.range[1], old, 'rq', factoryMap, fileParam);
		}
	},

	onRequireEnsure: function(node, factoryMap, fileParam, patches) {
		var self = this;
		...
		if (args[0].type === 'ArrayExpression') {
			args[0].elements.forEach(nameNode => {
				...
				self.addPatch(patches, nameNode.range[0], nameNode.range[1], old, 'rs', factoryMap, fileParam);
			});
		} else if (args[0].type === 'Literal') {
			var old = _.get(node, 'arguments[0].value');
			self.addPatch(patches, args[0].range[0], args[0].range[1], old, 'rs', factoryMap, fileParam);
		}
	},

	addPatch: function(patches, start, end, moduleName, astType, factoryMap, fileParam) {
		var self = this;
